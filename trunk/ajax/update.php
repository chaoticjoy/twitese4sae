<?php
	...
	if ( isset($_POST['status']) && isset($_POST['in_reply_to']) ) {
		...
		if (isset($status->user)) {
			...
			$output .= "<a style=\"display:none\" class=\"replie_btn\"href=\"a_reply.php?id=$status->id\">回复</a>";
			$output .= "<a style=\"display:none\" class=\"rt_btn\" href=\"a_rt.php?id=$status->id\">回推</a>";
			$output .= "<a style=\"display:none\" class=\"favor_btn\" href=\"a_favor.php?id=$status->id\">收藏</a>";
			$output .= "<a style=\"display:none\" class=\"delete_btn\" href=\"a_del.php?id=$status->id&t=s\">删除</a>";
