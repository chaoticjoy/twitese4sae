	...
	<div id="setting">
		...
		<?php
			...
			if ( isset($_POST['apiurl_input']) ) {
				...
				setcookie("rt_style", $_POST['rt_style'], $time,"/",COOKIE_DOMAIN);
				$rtStyle = $_POST['rt_style'];

				if (preg_match("/^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"])*$/", $_POST['apiurl_input'])) {
					$time = time() + 3600*24*30;

					setcookie("apiurl", $_POST['apiurl_input'], $time,"/",COOKIE_DOMAIN);
					$apiUrl = $_POST['apiurl_input'];
